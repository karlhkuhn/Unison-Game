...
class World1Level6: SKScene, SKPhysicsContactDelegate {
	...
	override func didMoveToView(view: SKView) {
		...
		movingTileOffScreenLeftPositionX = matrixGridLeftReferencePoints["A1"]!.x - (tileSide * 1.5)
		movingTileOffScreenRightPositionX = matrixGridRightReferencePoints["E1"]!.x + (tileSide * 1.5)
		movingTileOffScreenTopPositionY = matrixGridLeftReferencePoints["A1"]!.y + (tileSide * 1.5)
		movingTileOffScreenBottomPositionY = matrixGridLeftReferencePoints["A1"]!.y - (tileSide * 1.5)
	...
	func makeAVerticalWall(atStartPosition startPosition: CGPoint) {
		let wall4 = Wall(texture: SKTexture(imageNamed: "wallObstacle"), color: UIColor.clearColor(), size: CGSize(width: tileSide * 0.2, height: tileSide))
		wall4.position = CGPoint(x: matrixGridLeftReferencePoints["C1"]!.x, y: matrixGridLeftReferencePoints["C1"]!.y + (tileSide * 1.5))
		playingBoardNode.addChild(wall4)
		
//		wall4.moveWallForever(MovingWallStartSide)
	}
	
	func createWall(startSide: MovingWallStartSide, atStartPosition position: String) {
		
		//HorizontalWall
		if startSide == MovingWallStartSide.Left || startSide == MovingWallStartSide.Right {
			let wall = Wall(texture: SKTexture(imageNamed: "wallObstacle"), color: UIColor.clearColor(), size: CGSize(width: tileSide, height: tileSide * 0.2))
			if startSide == .Left {
		...
		if startSide == MovingWallStartSide.Top || startSide == MovingWallStartSide.Bottom {
			let wall = Wall(texture: SKTexture(imageNamed: "wallObstacle"), color: UIColor.clearColor(), size: CGSize(width: tileSide * 0.2, height: tileSide))
			if startSide == .Top {
...
//		
//		let wall = Wall(texture: SKTexture(imageNamed: "wallObstacle"), color: UIColor.clearColor(), size: CGSize(width: tileSide * 0.2, height: tileSide))
//		wall.position = CGPoint(x: matrixGridLeftReferencePoints["A2"]!.x, y: matrixGridLeftReferencePoints["A2"]!.y + (tileSide * 0.70))
//		playingBoardNode.addChild(wall)
//		
//		wall.moveWallUpAndDown(onGrid: .Player, fromPosition: "A2", toPosition: "A3")
		
		let spacer: NSTimeInterval = 0.43
		
		playingBoardNode.runAction(SKAction.repeatActionForever(SKAction.sequence([

			SKAction.runBlock({self.createWall(.Left, atStartPosition: "A3")}),
			SKAction.waitForDuration(spacer),
			SKAction.runBlock({self.createWall(.Left, atStartPosition: "A2")}),
			SKAction.runBlock({self.createWall(.Left, atStartPosition: "A4")}),
			SKAction.waitForDuration(spacer),
			SKAction.runBlock({self.createWall(.Left, atStartPosition: "A3")}),
			SKAction.waitForDuration(spacer),
			SKAction.runBlock({self.createWall(.Left, atStartPosition: "A2")}),
			...
			SKAction.runBlock({self.createWall(.Left, atStartPosition: "A2")}),
			SKAction.runBlock({self.createWall(.Left, atStartPosition: "A4")}),
			SKAction.waitForDuration(spacer * 2)
		])))
		
		playingBoardNode.runAction(SKAction.repeatActionForever(SKAction.sequence([
			
			SKAction.runBlock({self.createWall(.Top, atStartPosition: "C1")}),
			SKAction.waitForDuration(spacer * 4),
			SKAction.runBlock({self.createWall(.Top, atStartPosition: "C1")}),
			SKAction.waitForDuration(spacer * 4),
			])))
...
//		playingBoardNode.runAction(SKAction.sequence([
//			SKAction.waitForDuration(0.75),
//			SKAction.runBlock(makeAVerticalWall),
//			SKAction.waitForDuration(0.75),
//			SKAction.runBlock(makeAVerticalWall),
//			SKAction.waitForDuration(0.75),
//			SKAction.runBlock(makeAVerticalWall)
