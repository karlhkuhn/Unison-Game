...
class Tutorial3: SKScene, SKPhysicsContactDelegate {
	...
	override init (size: CGSize) {
		...
		startOffScreen = SKAction.moveToX(-2000 , duration: 0)
		moveToCenterOfScreen = SKAction.moveToX(size.width / 2, duration: 0.7)
		moveToOffScreen = SKAction.moveToX(playableRect.size.width + 2000 , duration: 0.7)
		
		moveToCenterOfScreen.timingMode = .EaseOut
		moveToOffScreen.timingMode = .EaseIn
		
		currentTutorialBox = .TutorialBox1
	...
	override func didMoveToView(view: SKView) {
		...
		setupTutorialBoxes()
		presentNextTutorialBox(.TutorialBox1)
	}
	...
	// MARK: UI SETUPS
	func setupTutorialBoxes() {
		tutorialBox1 = TutorialBox(theText: "Modifier Objects allow JimJam to make special movments in relation to Filo")
		tutorialBox1.position = CGPoint(x: -tutorialBox1.tutorialBoxBackground.size.width * 2, y: playableRect.size.height / 2)
		uiTutorialNode.addChild(tutorialBox1)
		
		tutorialBox2 = TutorialBox(theText: "This is a \"Doubler\".  It allows JimJam to move 2 blocks for every 1 that Filo moves...")
		tutorialBox2.position = CGPoint(x: -tutorialBox1.tutorialBoxBackground.size.width * 2, y: playableRect.size.height / 2)
		uiTutorialNode.addChild(tutorialBox2)
		
		tutorialBox3 = TutorialBox(theText: "and this is an \"Oppositer\".  It allows JimJam to move in the opposite direction of Filo")
		tutorialBox3.position = CGPoint(x: -tutorialBox1.tutorialBoxBackground.size.width * 2, y: playableRect.size.height / 2)
		uiTutorialNode.addChild(tutorialBox3)

		tutorialBox4 = TutorialBox(theText: "Now see if you can sove this puzzle using these modifiers!")
		tutorialBox4.position = CGPoint(x: -tutorialBox1.tutorialBoxBackground.size.width * 2, y: playableRect.size.height / 2)
		uiTutorialNode.addChild(tutorialBox4)

		//		tutorialBox5 = TutorialBox(theText: "Try swiping Filo to get him to the Blue Goal, and watch JimJam follow!")
		//		tutorialBox5.position = CGPoint(x: -tutorialBox1.tutorialBoxBackground.size.width * 2, y: playableRect.size.height / 2)
		//		uiTutorialNode.addChild(tutorialBox5)
		
	}
	
	func presentNextTutorialBox(theTutorialBox: TutorialBoxNumber) {
		
		
		switch theTutorialBox {
			
		case .TutorialBox1: tutorialBox1.runAction(moveToCenterOfScreen)
			switchToTutorial()
			
		case .TutorialBox2: tutorialBox1.runAction(SKAction.sequence([
			moveToOffScreen,
			SKAction.runBlock(tutorialBox1.removeFromParent)
			]))
		
		tutorialBox2.runAction(moveToCenterOfScreen)
		doubleMover.modifier.runAction(SKAction.group([
			SKAction.scaleTo(3, duration: 0.7),
			SKAction.moveTo(CGPoint(x: playableRect.size.width * 0.5, y: playableRect.size.height * 0.85), duration: 0.7)
			]))
		currentTutorialBox = .TutorialBox2
		print("Show Tut 2")
			
		case .TutorialBox3: tutorialBox2.runAction(SKAction.sequence([
			moveToOffScreen,
			SKAction.runBlock(tutorialBox2.removeFromParent)
			]))
			tutorialBox3.runAction(moveToCenterOfScreen)
			doubleMover.modifier.runAction(SKAction.group([
				SKAction.scaleTo(1, duration: 1),
				SKAction.moveTo(matrixGridLeft["I2"]!, duration: 1)
				]))
			oppositer.modifier.runAction(SKAction.group([
				SKAction.scaleTo(3, duration: 0.7),
				SKAction.moveTo(CGPoint(x: playableRect.size.width * 0.5, y: playableRect.size.height * 0.85), duration: 0.7)
				]))

		currentTutorialBox = .TutorialBox3
		print("Show Tut 3")

		case .TutorialBox4: tutorialBox3.runAction(SKAction.sequence([
			moveToOffScreen,
			SKAction.runBlock(tutorialBox3.removeFromParent)
			]))
		tutorialBox4.runAction(moveToCenterOfScreen)
		
		oppositer.modifier.runAction(SKAction.group([
			SKAction.scaleTo(1, duration: 0.7),
			SKAction.moveTo(matrixGridLeft["J2"]!, duration: 1)
			]))
			currentTutorialBox = .TutorialBox4

			//		case .TutorialBox5: tutorialBox4.runAction(SKAction.sequence([
			//			moveToOffScreen,
			//			SKAction.runBlock(tutorialBox4.removeFromParent)
			//			]))
			//		tutorialBox5.runAction(moveToCenterOfScreen)
			//		currentTutorialBox = .TutorialBox5
		//
		default: tutorialBox4.runAction(SKAction.sequence([
			moveToOffScreen,
			SKAction.runBlock(tutorialBox4.removeFromParent),
			SKAction.runBlock(switchToPlay)
			]))
	...
	func setupBorders() {
		...
		CGPathMoveToPoint(path, nil, matrixGridLeftReferencePoints["A1"]!.x, matrixGridLeftReferencePoints["A1"]!.y)
		CGPathAddLineToPoint(path, nil, matrixGridLeftReferencePoints["E1"]!.x, matrixGridLeftReferencePoints["E1"]!.y)
		CGPathAddLineToPoint(path, nil, matrixGridLeftReferencePoints["E5"]!.x, matrixGridLeftReferencePoints["E5"]!.y)
		CGPathAddLineToPoint(path, nil, matrixGridLeftReferencePoints["A5"]!.x, matrixGridLeftReferencePoints["A5"]!.y)
		CGPathAddLineToPoint(path, nil, matrixGridLeftReferencePoints["A1"]!.x, matrixGridLeftReferencePoints["A1"]!.y)
		shape.path = path
		...
		CGPathMoveToPoint(path2, nil, matrixGridRightReferencePoints["A1"]!.x, matrixGridRightReferencePoints["A1"]!.y)
		CGPathAddLineToPoint(path2, nil, matrixGridRightReferencePoints["E1"]!.x, matrixGridRightReferencePoints["E1"]!.y)
		CGPathAddLineToPoint(path2, nil, matrixGridRightReferencePoints["E5"]!.x, matrixGridRightReferencePoints["E5"]!.y)
		CGPathAddLineToPoint(path2, nil, matrixGridRightReferencePoints["A5"]!.x, matrixGridRightReferencePoints["A5"]!.y)
		CGPathAddLineToPoint(path2, nil, matrixGridRightReferencePoints["A1"]!.x, matrixGridRightReferencePoints["A1"]!.y)
		shape2.path = path2
...
//			[
//				matrixGridLeftReferencePoints["C1"]!,
//				matrixGridLeftReferencePoints["C2"]!
//			])
//		playingBoardNode.addChild(wall2)
		
		//		let oppositer1 = ModifierObject(modifierType: ModifierType.Oppositer, on: Grid.Player, at: "I3")
		//		playingBoardNode.addChild(oppositer1)
		//
		//		let doubleMover1 = ModifierObject(modifierType: ModifierType.DoubleMover, on: Grid.Mirror, at: "I2")
		...
		oppositer = ModifierObject(modifierType: ModifierType.Oppositer, on: .Player, at: "J2")
		playingBoardNode.addChild(oppositer)
		
		doubleMover = ModifierObject(modifierType: ModifierType.DoubleMover, on: .Player, at: "I2")
		playingBoardNode.addChild(doubleMover)
	...
	func setupLeftGrid() {
		...
		playingBoardNode.enumerateChildNodesWithName("YellowTileBlock", usingBlock: {node, stop in
	...
	func setupRightGrid() {
		...
		playingBoardNode.enumerateChildNodesWithName("SilverTileBlock", usingBlock: {node, stop in
			node.runAction(SKAction.repeatActionForever(SKAction.sequence([
				self.scaleUp, self.scaleDown
				])))})
	}
	...
	func advanceToNextLevel() {
		...
		let scene = World1Level3(size: gameSize)
	...
	func didRestartLevel() {
		let scene = Tutorial3(size: gameSize)
	...
	//		if let touch = touches.first {
	//			let targetNodes = self.nodesAtPoint(touch.locationInNode(self))
	//
	//			for targetNode in targetNodes {
	//				if targetNode.name == "HighlightBox" {
	//					let theTile = targetNode.parent as! TileObject
	//					theTile.removeHighlight()
	//					print("removed Highlight")
	//				} else {
	//					enumerateChildNodesWithName("YellowTile", usingBlock: {node, stop in
	//						let theTile = node as! TileObject
	//						theTile.removeHighlight()
	//					})
	//				}
	//
	...
	func sceneTouched(location: CGPoint) {
		//1
		let targetNodes = self.nodesAtPoint(location)
		
		switch currentTutorialBox {
		case .TutorialBox1: presentNextTutorialBox(.TutorialBox2)
		case .TutorialBox2: presentNextTutorialBox(.TutorialBox3)
					case .TutorialBox3: presentNextTutorialBox(.TutorialBox4)
//				case .TutorialBox4: presentNextTutorialBox(.TutorialBox5)
		default: presentNextTutorialBox(.TutorialBox0)
		}
		print(currentTutorialBox)
		...
		for targetNode in targetNodes {
			...
			if gameState == .Play {
				...
				//				if targetNode.name == "YellowTile" {
				//					let theTile = targetNode.parent as! TileObject
				//					theTile.addHighlight()
				//					print("touched yellowTile")
				//				}
	...
	*/
		...
		if gameState == GameState.Play {
			...
			if collision == PhysicsCategory.MirrorPlayer | PhysicsCategory.Goal {
				if playerHitGoal == true {
					...
					defaults.setInteger(swipesMade, forKey: "Swipes-Tutorial3")
	...
	var currentTutorialBox: TutorialBoxNumber = .TutorialBox1
	
	var player = Player()
	var mirrorPlayer = Player()
	
	var tutorialBox1 = TutorialBox()
	var tutorialBox2 = TutorialBox()
	var tutorialBox3 = TutorialBox()
	var tutorialBox4 = TutorialBox()
	var tutorialBox5 = TutorialBox()
	...
	var oppositer = ModifierObject()
	var doubleMover = ModifierObject()
